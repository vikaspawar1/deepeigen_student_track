...
const ChatWidget: React.FC<ChatWidgetProps> = ({ mode = 'student' }) => {
  ...
  useEffect(() => {
    const initializeChatbot = async () => {
      try {
        if (mode === 'public') {
          const homeRes = await api.get('/api/chatbot/public-home/');
          setSuggestions(homeRes.data.suggested_questions || []);
          setStudentName('');
          setMessages([]); // no history for guests

          setTimeout(() => {
            setPopupReminder({
              message: "👋 Hi there! Curious about our courses or pricing? Ask me anything!",
              link: ''
            });
          }, 2000);
  ...
  const handleSendMessage = async (text: string) => {
    const userMsg: MessageType = { id: `usr-${Date.now()}`, sender: 'user', text };
    setMessages(prev => [...prev, userMsg]);
    setIsTyping(true);

    try {
      const endpoint = mode === 'public' ? '/api/chatbot/public-message/' : '/api/chatbot/message/';
  ...
  return (
    <>
      ...
      >
        ...
        <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 cursor-pointer text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          {isOpen ? (
